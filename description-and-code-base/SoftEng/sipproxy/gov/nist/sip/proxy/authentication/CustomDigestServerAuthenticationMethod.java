...
public class CustomDigestServerAuthenticationMethod implements
	 ...
	 * Default constructor.
	 */
	public CustomDigestServerAuthenticationMethod() {
		try {
	...
	public void initialize(String pwFileName) {
		try {
				...
				for (int i = 0; i < usersTagList.size(); i++) {
					...
					if (userName != null) {
						...
						if (userPassword == null) {
							ProxyDebug
									.println("DEBUG, DigestAuthenticationMethod, initialize(),"
											+ " the userPassword parameter does not exist for user: "
											+ userName
											+ ", we use the default: \""
											+ NULL_PASSWORD + "\"");
							userPassword = NULL_PASSWORD;
						}
						passwordTable.put(userName + "@" + DEFAULT_REALM,
								userPassword);
					} else {
						ProxyDebug
								.println("DEBUG, DigestAuthenticationMethod, initialize(),"
			...
			ProxyDebug
					.println("ERROR, DigestAuthenticationMethod, initialize(),"
							+ "exception raised:");
			e.printStackTrace();
		}
	}
